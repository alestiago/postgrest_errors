...
if [ ! -f "pubspec.yaml" ]; then
  echo "$(pwd) is not a valid package."
  exit 1
fi
...
previousTag="postgrest_errors_dart_package-${old_version}"
raw_commits="$(git log --pretty=format:"%s" --no-merges --reverse $previousTag..HEAD -- .)"
markdown_commits=$(echo "$raw_commits" | sed -En "s/\(#([0-9]+)\)/([#\1](https:\/\/github.com\/alestiago\/postgrest_errors\/pull\/\1))/p")
...
# Update pubspec.yaml file with new version.
